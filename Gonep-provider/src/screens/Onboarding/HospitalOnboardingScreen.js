...
export function HospitalOnboardingScreen({ onComplete }) {
  const { C } = useTheme();
  const [step, setStep] = useState(0);
  const [form, setForm] = useState({
    name: '', email: '', phone: '', location: '', registration_no: '', admin_name: '', admin_email: '',
  });
  ...
  const validateStep1 = () => {
    const required = DOC_TYPES.filter(d => d.required).map(d => d.id);
    return required.every(id => docs[id]);
  };
        ...
        <View style={styles.inner}>
          ...
          <View style={styles.stepRow}>
            {STEPS.map((s, i) => (
              <View key={s} style={styles.stepWrap}>
                ...
                <Text style={[styles.stepLabel, { color: i === step ? C.text : C.textMuted }]}>{s}</Text>
                {i < STEPS.length - 1 && (
          ...
          {step === 1 && (
            <View style={[styles.card, { backgroundColor: C.card, borderColor: C.border }]}>
              ...
              <Text style={[styles.cardDesc, { color: C.textMuted }]}>
                All documents are reviewed by the Gonep compliance team within 2–3 business days. Required documents are marked with an asterisk.
              </Text>
                ...
                return (
                  <View key={doc.id} style={[styles.docRow, { borderColor: uploaded ? C.success : C.border, backgroundColor: uploaded ? C.successLight : C.surface }]}>
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={[styles.docDesc, { color: C.textMuted }]}>
                        {uploaded ? `Uploaded: ${uploaded.name}` : doc.desc}
                      </Text>
                    </View>
                    <TouchableOpacity
                    ...
                    >
                      ...
                      <Text style={{ color: uploaded ? C.success : C.primary, fontSize: 12, fontWeight: '600' }}>
            ...
            <View style={[styles.card, { backgroundColor: C.card, borderColor: C.border }]}>
              <View style={styles.successWrap}>
                ...
                <Text style={[styles.successNote, { color: C.textMuted }]}>
                  Once approved, your administrator ({form.admin_email || 'admin'}) will receive login credentials and full access to the provider portal.
                </Text>
