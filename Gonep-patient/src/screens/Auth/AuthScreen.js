...
export function AuthScreen({ onAuth, appName }) {
  ...
  const submit = async () => {
    ...
    if (mode === 'login') {
      if (
        form.email.toLowerCase() === APP_CONFIG.DEMO_EMAIL &&
        form.password === APP_CONFIG.DEMO_PASSWORD
      ) {
        onAuth(MOCK_USER);
      } else {
        setGlobalErr('Invalid email or password. Try the demo credentials below.');
      }
    } else {
      onAuth({
        ...MOCK_USER,
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <View style={[styles.cardWrapper]}>
          <View style={styles.logoWrap}>
            ...
            <Text style={[styles.appSubtitle, { color: C.primary }]}>
              Patient Portal
            </Text>
          </View>
            ...
            >
              ...
              {globalErr ? (
                <View
                  style={[
                    styles.alert,
                    { backgroundColor: C.dangerLight, borderColor: C.danger, flexDirection: 'row', alignItems: 'center' },
                  ]}
                >
