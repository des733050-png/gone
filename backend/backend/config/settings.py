...
# The provider/patient web apps send Cache-Control/Pragma on GET requests.
# Add them to allowed request headers so CORS preflight succeeds.
CORS_ALLOW_HEADERS = (*default_headers, "cache-control", "pragma")

# CORS_ALLOWED_ORIGINS must list exact origins (scheme + host + port).
# Wildcard patterns such as https://*.example.com are not valid here.
